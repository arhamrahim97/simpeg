    ...
    <div class="col-xl-7 col-sm-12 mb-5">
        <div>
            ...
            <select class="form-select form-select-lg mb-3 col-lg-12" aria-label=".form-select-lg example"
                ...
                <option value="{{$pangkat->id}}" @if ($pangkat->id == $usulanPangkat->pangkat_selanjutnya)
                    selected
        ...
        <div class="mt-4">
            <label>TMT Pangkat Selanjutnya (contoh: <span style="color: seagreen">01-12-2021</span>) :</label>
            <input name="tmt_pangkat_selanjutnya" type="text" class="form-control tanggal"
                placeholder="Masukkan TMT Pangkat Selanjutnya" required
                value="{{date("d-m-Y", strtotime($usulanPangkat->tmt_pangkat_selanjutnya))}}" readonly>
        </div>
        <div>
            <label for="exampleInputEmail1" class="form-label mt-4">Konfirmasi Berkas</label>
            <select class="form-select form-select-lg mb-3 col-lg-12" aria-label=".form-select-lg example"
                id="konfirmasi-berkas" name="konfirmasi_berkas" required disabled>
                <option value="" selected>Pilih Status Konfirmasi</option>
                <option value="1" @if ($usulanPangkat->status_kepegawaian == 1) {{ 'selected' }} @endif>Setuju
                </option>
                <option value="2" @if ($usulanPangkat->status_kepegawaian == 2) {{ 'selected' }} @endif>Tolak
                </option>
            </select>
        </div>

        <div id="form-alasan-ditolak" {{ ($usulanPangkat->status_kepegawaian != 2) ? 'hidden' : '' }}>
            <label for="exampleFormControlTextarea1" class="form-label">Alasan Ditolak</label>
            <textarea class="ckeditor form-control" id="ckeditor" required name="alasan_ditolak"
                readonly>{{$usulanPangkat->alasan_tolak_kepegawaian}}</textarea>
        </div>
        @endif
